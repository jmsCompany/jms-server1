...
public interface SCompanyCoRepository  extends JpaRepository<SCompanyCo, Long>{
	...
	@Query("select s from SCompanyCo s where s.company.idCompany=?1 and s.name=?2 and s.STypeDic.id=?3")
	public SCompanyCo findByCompanyIdAndNameAndType(Long idCompany,String name,Long idType);
	
	@Query("select s from SCompanyCo s where s.company.idCompany=?1 and s.name=?2")
	public SCompanyCo findByCompanyIdAndNameNoStatus(Long idCompany,String name);
