...
@Query("select s from SInventory s where s.SMaterial.idMaterial=?1 and  s.qty<>0 and s.SBin.SStk.SStkTypeDic.name in ('原材料仓','成品仓','来料检验仓')")
public List<SInventory> findByMaterialIds(Long materialId);
