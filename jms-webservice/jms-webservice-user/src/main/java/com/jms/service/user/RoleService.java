...
public class RoleService {
	...
	public Message addRole(WSRoles wsRoles) throws Exception
	{
		...
		return messagesUitl.getMessage("company.role.success",null,MessageTypeEnum.INFOMATION);
	}
	...
	public Message checkRole(WSRoles wsRoles)
	{
		...
		if(roleRepository.findByRoleAndCompanyName(wsRoles.getRole(), wsRoles.getCompanyName())==null)
			return messagesUitl.getMessage("company.role.avaible",null,MessageTypeEnum.INFOMATION);
		 return messagesUitl.getMessage("company.role.alreadyexist",null,MessageTypeEnum.ERROR);
	}
	...
	public Message addRolePrivs(WSRolePrivs wsRolePrivs)
	{
		...
		Roles roles=roleRepository.findByRoleAndCompanyName(role, companyName);
		if(roles==null)
			return  messagesUitl.getMessage("company.role.doesnotexist",null,MessageTypeEnum.ERROR);
		...
		for(Modules mod: modulesRepository.findAll())
		{
			...
			if(rolePriv!=null)
			{
				int idx = checkModuleInTheList(mod.getName(),wsRolePrivs.getModulePrivList());
				if(idx!=-1)
				{
					rolePriv.setPriv(wsRolePrivs.getModulePrivList().get(idx).getPriv());
				}
				else
				{
					rolePriv.setPriv(0);
		...
		return messagesUitl.getMessage("company.role.priv.success",null,MessageTypeEnum.INFOMATION);
	}
	...
	private int checkModuleInTheList(String module,List<WSModulePriv> wsModulePrivList)
	{
		int idx=0;
